...
(function() {
  var Base, GenericBinding, SHA256, Verifier, WordArray, add_ids, akatch, base64_extract, base64u, bufeq_secure, cieq, constants, decode, hash_sig, iced, json_stringify_sorted, katch, make_esc, make_ids, pgp_utils, sig_id_to_short_id, streq_secure, triplesec, unix_time, util, __iced_k, __iced_k_noop, _ref,
  ...
  Base = (function() {
    ...
    Base.prototype.sanity_check_proof_text = function(_arg, cb) {
      var args, b, b64s, check_for, err, lim, msg, proof_text, _i, _len, _ref1;
      args = _arg.args, proof_text = _arg.proof_text;
      if (this.is_short()) {
        check_for = args.sig_id_short;
        lim = constants.short_id_bytes;
      } else {
        _ref1 = decode(args.sig), err = _ref1[0], msg = _ref1[1];
        if ((err == null) && (msg.type !== "MESSAGE")) {
          err = new Error("wrong message type; expected a generic message; got " + msg.type);
        }
        if (err == null) {
        ...
        for (_i = 0, _len = b64s.length; _i < _len; _i++) {
          ...
          if (b.length >= lim) {
            if (b.indexOf(check_for) !== 0) {
              err = new Error("Found a bad hash in proof text: " + b.slice(0, 40) + " != " + check_for.slice(0, 40));
              break;
            }
          }
        }
