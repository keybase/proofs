...
(function() {
  var BaseScraper, b64find, constants, decode_sig, iced, ipaddress, pkg, sncmp, space_normalize, urlmod, user_agent, v_codes, __iced_k, __iced_k_noop;
  ...
  BaseScraper = (function() {
    ...
    BaseScraper.prototype._get_url_body = function(opts, cb) {
      var body, err, followRedirect, rc, response, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      ...
      opts.headers["User-Agent"] = opts.user_agent || user_agent;
      followRedirect = (function(_this) {
        return function(response) {
          var fail, hostname, port, search, _ref;
          _ref = urlmod.parse(response.headers.location), hostname = _ref.hostname, port = _ref.port, search = _ref.search;
          fail = function(why) {
            _this.log("Failure in redirect path for " + _this.hostname + ": " + why);
            return false;
          };
          if (!(hostname != null ? hostname.length : void 0)) {
            return fail("no hostname");
          }
          if ((port != null) && !(port === 80 || port === 443)) {
            return fail("bad port: " + port);
          }
          if (new ipaddress.Address4(hostname).isValid()) {
            return fail("found an IPv4 address (" + hostname + ")");
          }
          if (new ipaddress.Address6(hostname).isValid()) {
            return fail("found an IPv6 address (" + hostname + ")");
          }
          if (search != null) {
            return fail("found a search parameter (" + search + ")");
          }
          return true;
        };
      })(this);
