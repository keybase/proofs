...
(function() {
  ...
  exports.OuterLink = OuterLink = (function() {
    ...
    OuterLink.decode = function(obj) {
      var err, schm;
      schm = schema.struct([schema.value(3).name("version"), schema.seqno().name("seqno"), schema.binary(32).name("prev").optional(), schema.binary(32).name("inner_link"), schema.link_type().name("link_type"), schema.chain_type().name("chain_type"), schema.bool().name("ignore_if_unsupported")]).name("outer");
  ...
  exports.Base = Base = (function() {
    function Base(_arg) {
      this.sig_eng = _arg.sig_eng, this.seqno = _arg.seqno, this.user = _arg.user, this.prev = _arg.prev, this.client = _arg.client, this.merkle_root = _arg.merkle_root, this.ignore_if_unsupported = _arg.ignore_if_unsupported, this.ctime = _arg.ctime, this.entropy = _arg.entropy, this.parent_chain_tail = _arg.parent_chain_tail, this.new_sig_km = _arg.new_sig_km;
    }
